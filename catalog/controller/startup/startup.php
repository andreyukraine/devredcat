...
class ControllerStartupStartup extends Controller {
    ...
    if (preg_match('/\.(?:jpg|jpeg|png|gif|ico|css|js|svg)$/', $_SERVER['REQUEST_URI'])) {
      return false; // Зупинити виконання PHP для статичних файлів
    }
    ...
    if (!isset($this->session->data['catalog_type'])) {
      $this->session->data['catalog_type'] = (int)$this->config->get('config_category_type');
    }

    if (!isset($this->request->cookie['catalog_type'])) {
      setcookie('catalog_type', (int)$this->config->get('config_category_type'), time() + 60 * 60 * 24 * 30, '/', $this->request->server['HTTP_HOST']);
    }
    ...
    require_once DIR_SYSTEM . 'library/md/mobile_detect.php';
    $this->registry->set('mobile_detect', new jc_Mobile_Detect());
  }
}
