...
class ControllerStartupSeoUrl extends Controller
{
  ...
  public function index()
  {
    ...
    if (isset($this->request->get['_route_'])) {
      ...
      $filter_pos = strpos($route, '/filter/');

      if ($filter_pos !== false) {
        // Отримуємо шлях до фільтра
        $parts = explode('/', substr($route, 0, $filter_pos));

        // Отримуємо параметри фільтра
        $filter_params_str = substr($route, $filter_pos + 8);

        $this->request->get['filter'] = $filter_params_str;
      }
      ...
      foreach ($parts as $part) {
        ...
        $sql = "SELECT * FROM " . DB_PREFIX . "seo_url WHERE keyword = '" . $this->db->escape($part) . "' AND store_id = '" . (int)$this->config->get('config_store_id') . "'";
        if ($is_brand) {
    ...
    if ($data) {
      ...
      foreach ($data as $key => $value) {
        if ($key === "filter") {
          ...
          $queryParts[] = rawurlencode((string)$key) . '=' . rawurlencode(is_array($value) ? http_build_query($value) : (string)$value);
        }
      }

      if (!$hasFilter && !empty($queryParts)) {
        $query = '?' . str_replace('&', '&amp;', implode('&', $queryParts));
      } elseif (!empty($queryParts)) {
        // Якщо є і filter, і звичайні параметри, можна додати ? після filter
        $query .= '?' . str_replace('&', '&amp;', implode('&', $queryParts));
      }
    }
