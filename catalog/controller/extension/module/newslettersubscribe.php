...
class ControllerExtensionModuleNewslettersubscribe extends Controller
{
  ...
  public function index($setting)
  {
    ...
    if ($this->config->get('config_account_id')) {
      ...
      if ($information_info) {
        $data['text_agree'] = '<a class="policy bl-bold" href="' . $this->url->link('information/information', 'information_id=' . (int)$information_info['information_id']) . '">Детальніше</a>';
      } else {
        $data['text_agree'] = '';
      }
    } else {
  ...
  public function subscribe()
  {
    ...
    if (isset($this->request->post['subscribe_email']) and filter_var($this->request->post['subscribe_email'], FILTER_VALIDATE_EMAIL)) {
      ...
      if ($this->config->get('newslettersubscribe_registered') and $this->model_account_newslettersubscribe->checkRegisteredUser($this->request->post)) {
        ...
        echo('$("' . $prefix_eval . ' #notification-error").html("<div class=\"success\"> ' . $this->language->get('subscribe') . '</div>");$("' . $prefix_eval . ' #subscribe-normal")[0].reset();');


      } else if (!$this->model_account_newslettersubscribe->checkmailid($this->request->post)) {

        if (isset($this->request->post['agree-policy'])) {
          ...
          echo('$("' . $prefix_eval . ' #notification-error").html("<div class=\"text-danger\"> ' . $this->language->get('error_polici') . '</div>")');
        }
      } else {
        echo('$("' . $prefix_eval . ' #notification-error").html("<div class=\"text-danger\"> ' . $this->language->get('alreadyexist') . '</div>");$("' . $prefix_eval . ' #subscribe-normal")[0].reset();');
      }
    } else {
      echo('$("' . $prefix_eval . ' #notification-error").html("<div class=\"text-danger\"> ' . $this->language->get('error_invalid') . '</div>")');
    }
