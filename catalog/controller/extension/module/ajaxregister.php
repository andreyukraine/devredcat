...
class ControllerExtensionModuleAjaxregister extends Controller
{
  ...
  public function index()
  {
    ...
    $data['button_register_link'] = $this->language->get('text_register');
    ...
    $data['show_customer_groups'] = false;

    if (isset($this->request->post['customer_type'])){

      $data['show_customer_groups'] = true;
      switch ((int)$this->request->post['customer_type']){
        case 1:
        ...
        case 2:
          if (is_array($this->config->get('config_customer_group_display'))) {
            ...
            foreach ($customer_groups as $customer_group_item) {
              //якщо зовнышній код 286 - це інтернет магазин
              if ($customer_group_item['guid'] == 286){
                $customer_group = $customer_group_item;
                break;
              }
            }
          }
          break;
        case 3:
          $customer_group = $this->model_account_customer_group->getCustomerGroupByUid('1');
          break;
      }
  ...
  public function register()
  {
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
      ...
      // Виклик пошти клієнту та адміністратору (OpenCart 3 events manual trigger)
      $this->load->controller('mail/register', $this->request->post);
      $this->load->controller('mail/register/alert', $this->request->post);

      //треба перевірка після регістрації
      $approval = 0;

      if (!empty($this->request->post['customer_group_id'])) {
        $this->load->model('account/customer_group');
        $customer_group = $this->model_account_customer_group->getCustomerGroup((int)$this->request->post['customer_group_id']);
        if ($customer_group != null) {
          $approval = (int)$customer_group['approval'];
        }
      }

      if ($customer_id != null) {
        $route = 'model/account/customer/addCustomer/after';
        $args = [$this->request->post];
        $output = $customer_id;
        $this->event->trigger($route, array(&$route, &$args, &$output));
      }

      // Clear any previous login attempts for unregistered accounts.
      $this->model_account_customer->deleteLoginAttempts($this->request->post['email']);

      $login_success = $this->customer->login($this->request->post['email'], $this->request->post['password']);

      if ($login_success){
        unset($this->session->data['guest']);
        // Якщо вдалося – редірект в особистий кабінет
        ...
        $json['success_html'] = $this->success();
      }else{
        $json['success'] = true;
        $json['success_message'] = $this->language->get('text_approval');
        $json['success_html'] = $this->approved();
      }
  ...
  public function validate()
  {
    ...
    foreach ($custom_fields as $custom_field) {
      if ($custom_field['location'] == 'account') {
        if ($custom_field['required'] && (empty($this->request->post['custom_field'][$custom_field['location']][$custom_field['custom_field_id']]) || (is_string($this->request->post['custom_field'][$custom_field['location']][$custom_field['custom_field_id']]) && trim($this->request->post['custom_field'][$custom_field['location']][$custom_field['custom_field_id']]) == ''))) {
  ...
  public function approved()
  {
    ...
    return $this->load->view('extension/module/ocajaxlogin/approved', $data);
  }
