<?php
class ControllerExtensionModuleCustomOrdersOrg extends Controller
{
  public function index($setting = array())
  {
    ...
    if (!empty($products)) {
      foreach ($products as $item) {
        ...
        if (!isset($ordersByStore[$organization_id])) {
          $ordersByStore[$organization_id] = [];
        }
      ...
      foreach ($ordersByStore as $organization_id => $items) {

        $org_info = $this->model_extension_module_ocorganization->getOrganizationId($organization_id);
        $total_order = 0;
        $session_products = [];
        ...
        foreach ($items as $prod) {
          ...
          $image = $prod['image'];
          if (!empty($prod['option_data'])) {
            foreach ($prod['option_data'] as $option) {
        ...
        $this->session->data['orders'][$organization_id] = $session_products;

        $data['orders'][$organization_id]['products'] = $session_products;
        if ($org_info != null) {
          $data['orders'][$organization_id]['warehouse'] = $org_info['name'];
        }else{
          $data['orders'][$organization_id]['warehouse'] = "Не вказаний склад";
        }
        $data['orders'][$organization_id]['total_order'] = $this->currency->format($total_order, $this->session->data['currency']);
    ...
    return $this->load->view('extension/module/custom/orders_org', $data);
  }
