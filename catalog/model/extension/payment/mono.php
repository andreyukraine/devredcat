...
class ModelExtensionPaymentMono extends Model
{
  ...
  public function addOrder($args)
  {
    ...
    if (!isset($args['order_id']) || !isset($args['randKey']) || !isset($args['InvoiceId'])) {
      throw new Exception('Missing required parameters');
    }

    // Проверяем и очищаем данные
    $order_id = (int)$args['order_id'];
    $randKey = $this->db->escape($args['randKey']);
    $invoiceId = $this->db->escape($args['InvoiceId']);

    // Проверяем существование записи
    $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "mono_orders` WHERE OrderId = '" . $order_id . "'");

    if ($query->num_rows) {
      // Обновляем существующую запись
      $this->db->query("UPDATE `" . DB_PREFIX . "mono_orders` 
                         SET SecretKey = '" . $randKey . "', 
                             InvoiceId = '" . $invoiceId . "' 
                         WHERE OrderId = '" . $order_id . "'");
    } else {
      // Создаем новую запись
      $this->db->query("INSERT INTO `" . DB_PREFIX . "mono_orders` 
                         (InvoiceId, OrderId, SecretKey) 
                         VALUES('" . $invoiceId . "', '" . $order_id . "', '" . $randKey . "')");
    }
  ...
  public function getEncodedProducts($order_id)
  {
    ...
    foreach ($orderProducts as $orderProduct) {
      ...
      $total_line = $this->currency->format($orderProduct['price'], $orderInfo['currency_code'], (int)$orderInfo['currency_value'], false);
      $total_temp = 0;

      $img = "no_image.png";
      $img_prod = $this->getImageUrl($orderProduct['product_id']);
      if (!empty($img_prod)){
        $img = $img_prod['image'];
      }
      ...
      $products[] = [
        ...
        'sum' => (int)round($total_line * 100),
        'qty' => (int)$orderProduct['quantity'],
        'icon' => HTTPS_SERVER . "/image/" . $img
      ];
    }
    return $products;
  }
