...
class ModelExtensionModuleDiscount extends Model
{
  protected $discount_modules = array('category', 'manufacturer', 'product', 'loyalty', 'customer_group');

  public function applyDisconts($product_id, $mass_opts)
  {
    ...
    $val_prod = $this->model_catalog_product->getProductOptPrice($product_id, $mass_opts);
    if (!empty($val_prod)) {

      $price_rrc = $val_prod["price"];
      $price = $val_prod["price"];
      $price_base = $val_prod["price_base"];
      $sku = $val_prod["sku"];
      $ean = $val_prod["ean"];
      $quantity = $val_prod["quantity"];
    }
    ...
    //Спочатку вирахуємо дисконт по програмі лояльності
    $discont_card = 0;
    if ($this->customer->isLogged()) {
      if ($this->config->get('total_loyalty_discount_status')) {
        $discounts['loyalty'] = $this->getLoyaltyDiscount();
        if (!empty($discounts['loyalty'])) {
          if ((int)$discounts['loyalty']['discount'] > 0) {
            $discont_card = (int)$discounts['loyalty']['discount'];
            if ($price > 0) {
              $prices['price'] = $price;
              $prices['tax'] = 0;
            }
    ...
    if ($this->customer->isLogged()) {
      ...
      $customer_price_id = 0;
      if ($client_address_id > 0) {
        $address = $this->model_account_address->getAddress($client_address_id);
        if (!empty($address)){
          $customer_price_id = $address['customer_price_id'];
        }
      }

      $customer_price = $this->model_account_customer_price->getCustomerPrice($customer_price_id);
      switch ($customer_price["guid"]){
        case "2":
          $prices['price'] = $prices['price_base'];
          break;
        default:
          $prices['price'] = $prices['price_rrc'];
      }

      if (isset($this->session->data['client_address_id'])) {
        $product = $this->model_catalog_product->getProduct($product_id);
        $address_id = (int)$this->session->data['client_address_id'];
        $price_groups = $this->model_account_customer->getPriceGroups($address_id, $this->customer->getId());
        if (!empty($price_groups) && $product != false) {
          foreach ($price_groups as $group) {
            if ($group['price_group_id'] == $product['price_group_id']) {
              $prices['percent'] = $group['percent'];
              $prices['special'] = $prices['price'] - ($prices['price'] * ($prices['percent'] / 100));
              break;
            }
          }
        }
    ...
    if ($this->config->get('total_product_discount_status')) {
      $discounts['product'] = $this->getAllProductDiscounts($product_id, $customer_group_id);
      if (!empty($discounts['product'])) {
        foreach ($discounts['product'] as $discont_product) {
          if ($mass_opts != null) {
            if ($discont_product['options_value'] == json_encode($mass_opts)) {
              if ((int)$discont_product['price'] > 0) {
                $prices['price'] = $discont_product['price'];
              }
              $prices['percent'] += $discont_product['percentage'];
              $prices['date_start'] = $discont_product['date_start'];
              $prices['date_end'] = $discont_product['date_end'];
              break;
            }
          } else {
            $prices['percent'] += $discont_product['percentage'];
        ...
        if ($prices['percent'] > $discont_card) {
          if ($prices['percent'] > 0) {
            $prices['special'] = $prices['price'] - ($prices['price'] * ($prices['percent'] / 100));
          }
        } else {
          if ($discont_card > 0) {
            $prices['percent'] += $discont_card;
            $prices['special'] = $prices['price'] - ($prices['price'] * ($prices['percent'] / 100));
          }
        }
      } else {
  ...
  public
  function getAllProductDiscounts($product_id, $customer_group_id = 1)
  {
    $sql = " SELECT * FROM " . DB_PREFIX . "product_discount pd";
    $sql .= " WHERE pd.product_id = '" . (int)$product_id . "' AND pd.customer_group_id = '" . (int)$customer_group_id . "'";
    $sql .= " ORDER BY pd.percentage, pd.quantity, pd.priority, pd.price";
    $query = $this->db->query($sql);
    return $query->rows;
  }

  public
  function getDiscountProducts($data = array(), $specials, $total)
  {

    $this->load->model('catalog/product');
    $special_override = $this->config->get('module_discounts_pack_override_special_price');
    ...
    foreach ($this->discount_modules as $module) {
      $key = 'total_' . $module . '_discount_status';
    ...
    if (!empty($sql)) {
      ...
      foreach ($query->rows as $result) {
        if ($special_override == 'exclusive') {
          $has_special_price = $this->hasSpecialPrice($result['product_id'], $customer_group_id);
          if (empty($has_special_price)) {
    ...
    if (isset($data['start']) || isset($data['limit'])) {
      ...
      $paginated_result = array_slice($product_data, $data['start'], $data['limit'], true);
    }

    return array($paginated_result, $total);
  }

  public
  function getCategoryDiscount($product_id)
  {
    ...
    if ($this->config->get('module_discounts_pack_override_special_price') == 'exclusive' && $this->hasSpecialPrice($product_id, $customer_group_id)) {
      return false;
    }

    $query = $this->db->query("SELECT ptc.category_id as category_id, cp.path_id as parent_id, cp.level FROM `" . DB_PREFIX . "product_to_category` ptc INNER JOIN `" . DB_PREFIX . "category_path` cp ON ptc.category_id = cp.category_id WHERE product_id = '" . (int)$product_id . "' ORDER BY cp.level DESC, ptc.category_id DESC");
    ...
    if ($query->num_rows > 1) {
      ...
      if (!empty($discount_query)) {
        ...
        arsort($high);
        reset($high); // TODO remove unnecessary statement
  ...
  protected
  function getMatchingCategoryDiscount($discount_query, $customer_group_id)
  {
  ...
  protected
  function getCategoryProductQty($discount, $customer_group_id)
  {
      ...
      foreach ($cart_query->rows as $cart) {
        ...
        foreach ($categories as $category) {
          ...
          if ($discount['category_id'] == $category) {
            ...
            $exclusive = $this->config->get('module_discounts_pack_override_special_price') == 'exclusive' ? !$this->hasSpecialPrice($product, $customer_group_id) : 1;
          ...
          if ($discount['affect']) {
            $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "category_path` WHERE `category_id` = '" . (int)$category . "' ORDER BY level DESC");
            $category_path_array = $query->rows;
            // remove first line
            array_shift($category_path_array);
            while (!empty($category_path_array)) {
              $parent_id = $category_path_array[0]['path_id'];
      ...
      foreach ($this->session->data['cart'] as $key => $quantity) {
        ...
        foreach ($categories as $category) {
          ...
          if ($discount['category_id'] == $category) {
            ...
            $exclusive = $this->config->get('module_discounts_pack_override_special_price') == 'exclusive' ? !$this->hasSpecialPrice($product['product_id'], $customer_group_id) : 1;
  ...
  public
  function getManufacturerDiscount($product_id)
  {
  ...
  protected
  function getManufacturerProductQty($manufacturer_id, $customer_group_id)
  {
    $manufacturer_qty = 0;
  ...
  public function getLoyaltyDiscounts()
  {
    $discounts = array();
    if (!$this->customer->isLogged()) {
      return $discounts;
      ...
      $table_check = $this->db->query("SHOW TABLES LIKE '" . DB_PREFIX . "loyalty_discount'");
      if ($table_check->num_rows) {
        $discounts_query = $this->db->query("SELECT percentage, ordertotal, order_status FROM `" . DB_PREFIX . "loyalty_discount` WHERE status = '1' AND customer_group_id = '" . (int)$customer_group_id . "' ORDER BY ordertotal ASC, priority ASC, percentage ASC");
        foreach ($discounts_query->rows as $key => $discount) {
          $discounts[] = array(
            "sum" => $discount["ordertotal"],
  ...
  public function getLoyaltyDiscount()
  {
    ...
    if (!$this->customer->isLogged()) {
      ...
      $loyalty_discount = array('discount' => 0, 'ordertotal' => 0);
      $customer_group_id = $this->customer->getGroupId();
...
//      $table_check = $this->db->query("SHOW TABLES LIKE '" . DB_PREFIX . "loyalty_discount'");
//      if ($table_check->num_rows) {
//        $discounts_query = $this->db->query("SELECT percentage as discount, ordertotal, order_status FROM `" . DB_PREFIX . "loyalty_discount` WHERE status = '1' AND customer_group_id = '" . (int)$customer_group_id . "' ORDER BY ordertotal ASC, priority ASC, percentage ASC");
//        $discounts = $discounts_query->rows;
//      }

      //cпочатку шукаэмо сумму по карты
      $card = false;
      ...
      if (!empty($discounts)) {
        foreach ($discounts as $key => $discount) {
          ...
          //якщо немає карти дивимось сумму по замовленням
          if (!$card) {
          ...
          if ($key == 0) {
            $loyalty_discount['discount'] = $discount['discount'];
            $loyalty_discount['ordertotal'] = $discount['ordertotal'];
          }

          if ($key < count($discounts) - 1) {
            if ($total > $discount['ordertotal'] && $total <= $discounts[$key + 1]['ordertotal']) {
              $loyalty_discount['discount'] = $discount['discount'];
              $loyalty_discount['ordertotal'] = $discount['ordertotal'];
              break;
            }
          } else {
            if ($total > $discount['ordertotal']) {
              $loyalty_discount['discount'] = $discount['discount'];
              $loyalty_discount['ordertotal'] = $discount['ordertotal'];
              break;
            }
  ...
  public
  function getCustomerDiscount($customer_id, $product_id)
  {
  ...
  public
  function getVolumeDiscount($product_id)
  {
    $this->load->model('catalog/product');
  ...
  protected
  function getVolumeProductQty($product_id, $customer_group_id)
  {
    $qty = 0;
  ...
  protected
  function hasDiscountPrice($product_id, $customer_group_id)
  {
  ...
  protected
  function hasSpecialPrice($product_id, $customer_group_id)
  {

    if (!$this->customer->isLogged() && $customer_group_id == 0) {
      $customer_group_id = $this->config->get('config_customer_group_id');
    }
  ...
  public
  function specialPriceOverride($product_id, $special_price, $standard_price, $discount_price, $customer_group_id)
  {

    $discount_override = $this->config->get('module_discounts_pack_override_discount_price');
    $special_override = $this->config->get('module_discounts_pack_override_special_price');
  ...
  protected
  function getDiscountsSpecialPrice($product_id, $customer_group_id)
  {

    $discounts = array();

    if ($this->config->get('total_category_discount_status')) {
      $query = $this->db->query("SELECT ptc.category_id as category_id, cp.path_id as parent_id, cp.level FROM `" . DB_PREFIX . "product_to_category` ptc INNER JOIN `" . DB_PREFIX . "category_path` cp ON ptc.category_id = cp.category_id WHERE product_id = '" . (int)$product_id . "' ORDER BY cp.level DESC, ptc.category_id ASC");
    ...
    if ($this->config->get('total_manufacturer_discount_status')) {
    ...
    if ($this->config->get('total_customer_group_discount_status')) {
      if ($this->customer->isLogged()) {
    ...
    if ($this->config->get('total_volume_discount_status')) {
  ...
  public
  function discountPriceOverride($product_id, $customer_group_id)
  {
      ...
      if (!empty($discounts)) {
        ...
        if ($discounts_tmp) {
          ...
          foreach ($i as $key) {
            ...
            $discount = $discounts_prices[$key]['price'] / 100 * $discount_percentage;
            ...
            $discounts[] = array(
              ...
              'price' => $discounts_prices[$key]['price'] - $discount
            );
          }
        }
      }
      ...
      if ($special_override == 'exclusive' && !empty($special_price)) {
        ...
        if (!empty($discounts)) {
          ...
          if ($discounts_tmp) {
            ...
            foreach ($i as $key) {
              $discount_percentage = $discounts_tmp[$key]['percentage'] > 100 ? 100 : $discounts_tmp[$key]['percentage'];
              $discount = $price / 100 * $discount_percentage;
  ...
  protected
  function createPercentageArray($discounts, &$i)
  {
  ...
  protected
  function createPricesArray($price, $discounts_prices)
  {
  ...
  protected
  function getDiscountsDiscountPrice($product_id, $customer_group_id, $i)
  {
    $discounts = array();
    if ($this->config->get('total_category_discount_status')) {
      $query = $this->db->query("SELECT ptc.category_id as category_id, cp.path_id as parent_id, cp.level FROM `" . DB_PREFIX . "product_to_category` ptc INNER JOIN `" . DB_PREFIX . "category_path` cp ON ptc.category_id = cp.category_id WHERE product_id = '" . (int)$product_id . "' ORDER BY cp.level DESC, ptc.category_id ASC");
  ...
  protected
  function getCategoryDiscountsArray($categories, $customer_group_id, $special = false, &$i = array())
  {
    ...
    if (count($categories) > 1) {
      ...
      if ($discount_query) {
        // TODO properly test this through
        if ($special) {
          $discounts['category'] = array(
            'quantity' => $discount_query['quantity'],
            'percentage' => $discount_query['percentage']
          );
        } elseif (array_key_exists('quantity', $discount_query) && array_key_exists('percentage', $discount_query)) {
          $discounts['category'][] = array(
            'quantity' => $discount_query['quantity'],
            'percentage' => $discount_query['percentage']
          );
          if (!in_array($discount_query['quantity'], $i)) {
            $i[] = $discount_query['quantity'];
          }
        } else {
          foreach ($discount_query as $discount) {
            $discounts['category'][] = array(
  ...
  protected
  function categoryDiscountQuery($category_id, $customer_group_id, $flag = true, $parent = false)
  {
  ...
  protected
  function categoryDiscountQuery2($category_id, $customer_group_id, $flag = false)
  {
  ...
  protected
  function calcSpecialPrice($discounts, $display_special, $price)
  {
    foreach ($discounts as $key => $discount) {
    ...
    if (!empty($this->config->get('module_discounts_pack_rounding'))) {
      if ($this->config->get('module_discounts_pack_rounding') != 'none') {
        $precision = !empty($this->config->get('module_discounts_pack_rounding_precision')) ? (int)$this->config->get('module_discounts_pack_rounding_precision') : 0;
        $mode = $this->config->get('module_discounts_pack_rounding') == 'up' ? PHP_ROUND_HALF_UP : PHP_ROUND_HALF_DOWN;
        $special_price = round((float)$special_price, $precision, $mode);
      }
    }
