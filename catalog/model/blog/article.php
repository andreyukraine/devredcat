...
class ModelBlogArticle extends Model {
  public function getArticle($article_id) {
    $query = $this->db->query("SELECT DISTINCT a.article_id, a.image, a.viewed, a.author, a.sort_order, a.status, a.date_available, a.date_added, a.date_modified, ad.language_id, ad.name, ad.description, ad.intro_text, ad.meta_title, ad.meta_description, ad.meta_keyword FROM " . DB_PREFIX . "article a LEFT JOIN " . DB_PREFIX . "article_description ad ON (a.article_id = ad.article_id) LEFT JOIN ". DB_PREFIX ."article_to_store ats ON (a.article_id = ats.article_id) WHERE a.article_id = '" . (int)$article_id . "' AND ad.language_id = '" . (int)$this->config->get('config_language_id') . "' AND ats.store_id = '". (int)$this->config->get('config_store_id') ."' AND a.status = '1' ");
  ...
  public function getArticles($data = array()) {
    ...
    $sql .= " LEFT JOIN " . DB_PREFIX . "article_to_store ats ON (a.article_id = ats.article_id) ";
    $sql .= " LEFT JOIN " . DB_PREFIX . "article_to_blog_category atbc ON (a.article_id = atbc.article_id) ";
    ...
    if (!empty($data['filter_blog_category_id'])){
      $sql .= " AND atbc.blog_category_id = '" . (int)$data['filter_blog_category_id']  . "'";
    }
    ...
    if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
      ...
      // Зміна: за замовчуванням сортування по даті додавання (новіші спочатку)
      $sql .= " ORDER BY a.date_added DESC";
    }

    if (isset($data['order']) && ($data['order'] == 'DESC')) {
      $sql .= ", LCASE(ad.name) DESC";
    } else {
      $sql .= ", LCASE(ad.name) ASC";
    }
