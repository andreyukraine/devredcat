...
class ModelAccountCustomerGroup extends Model {
  ...
  public function getCustomerGroupByUid($guid) {
    $query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "customer_group cg LEFT JOIN " . DB_PREFIX . "customer_group_description cgd ON (cg.customer_group_id = cgd.customer_group_id) WHERE cg.guid = '" . $this->db->escape($guid) . "' AND cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
    return $query->row;
  }
