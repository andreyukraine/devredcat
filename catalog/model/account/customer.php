...
class ModelAccountCustomer extends Model {
    ...
    if (isset($data['customer_group_id'])) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$customer_group_id . "', store_id = '" . (int)$this->config->get('config_store_id') . "', language_id = '" . (int)$this->config->get('config_language_id') . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', zone_id = '" . (int)$data['zone_id'] . "', custom_field = '" . $this->db->escape(isset($data['custom_field']['account']) ? json_encode($data['custom_field']['account']) : '') . "', salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', newsletter = '" . (isset($data['newsletter']) ? (int)$data['newsletter'] : 0) . "', ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "', status = '" . (int)!$customer_group_info['approval'] . "', date_added = NOW()");
  ...
  public function deleteCustomerAddressPriceGroupsByCustomer($customer_id)
  {
    $this->db->query("DELETE FROM `" . DB_PREFIX . "customer_address_price_groups` WHERE customer_id = '" . (int)$customer_id . "'");
  }

  public function getPriceGroupByUid($uid)
  {
    $query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "price_groups WHERE uid = '" . $uid . "'");
    return $query->row;
  }

  public function addPriceGroupe($guid, $name)
  {
    $current_date = date('Y-m-d H:i:s');
    $this->db->query("INSERT INTO `" . DB_PREFIX . "price_groups` SET uid = '" . $this->db->escape($guid) . "', name = '" . $this->db->escape($name) . "', status = 1, `date_added` = '" . $current_date . "', `date_modified` = '" . $current_date . "'");
    return $this->db->getLastId();
  }

  public function addCustomerAddressPriceGroup($item, $customer_id, $address_id, $price_group_id)
  {
    $current_date = date('Y-m-d H:i:s');
    $this->db->query("INSERT INTO `" . DB_PREFIX . "customer_address_price_groups` SET 
  ...
  public function getPriceGroups($address_id, $customer_id){
    $price_groups = array();
    if (!empty($address_id)){
      $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer_address_price_groups WHERE customer_id = '" . (int)$customer_id . "' AND address_id = '" . (int)$address_id . "'");
      foreach ($query->rows as $result) {

        $price_group_name = "";
        $price_group = $this->getPriceGroup($result['price_group_id']);
        if (!empty($price_group)){
          $price_group_name = $price_group['name'];
        }

        $price_groups[] = array(
          'price_group_id' => $result['price_group_id'],
  ...
  public function getPriceGroup($price_group_id){
    $query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "price_groups WHERE price_group_id = '" . (int)$price_group_id . "'");
    return $query->row;
  }

  public function getCustomerGroupByUid($uid)
  {
    $query = $this->query("SELECT DISTINCT * FROM " . DB_PREFIX . "customer_group c WHERE c.guid = '" . $this->db->escape($uid) . "'");
    return $query->row;
  }

	public function editCustomer($customer_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', zone_id = '" . (int)$data['zone_id'] . "', custom_field = '" . $this->db->escape(isset($data['custom_field']['account']) ? json_encode($data['custom_field']['account']) : '') . "' WHERE customer_id = '" . (int)$customer_id . "'");
	}
