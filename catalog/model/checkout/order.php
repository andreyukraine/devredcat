...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		shipping_house = '" . $this->db->escape($data['shipping_house']) . "', 
		shipping_apartment = '" . $this->db->escape($data['shipping_apartment']) . "', 
		shipping_address = '" . $this->db->escape($data['shipping_address']) . "', 
		shipping_method = '" . $this->db->escape($data['shipping_method']) . "', 
		shipping_code = '" . $this->db->escape($data['shipping_code']) . "', 
		address = '" . $this->db->escape($data['address']) . "', 
		shipping_level = '" . $this->db->escape($data['shipping_level']) . "', 
		shipping_city_id = '" . $this->db->escape($data['shipping_city_id']) . "', 
		shipping_post_id = '" . $this->db->escape($data['shipping_post_id']) . "', 
		shipping_address_id = '" . $this->db->escape($data['shipping_address_id']) . "', 
		shipping_post = '" . $this->db->escape($data['shipping_post']) . "', 
		shipping_poshtomat = '" . $this->db->escape($data['shipping_poshtomat']) . "', 
		shipping_poshtomat_id = '" . $this->db->escape($data['shipping_poshtomat_id']) . "', 
		shipping_comment = '" . $this->db->escape($data['shipping_comment']) . "', 
		shipping_ttn_file = '" . $this->db->escape($data['shipping_ttn_file']) . "', 
		client_address_id = '" . $this->db->escape($data['client_address_id']) . "', 
		customer_cod_guid = '" . $this->db->escape($data['customer_cod_guid']) . "', 
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "', `options` = '" . json_encode($product['options']) . "', options_value = '" . json_encode($product['options_value']) . "'");
	...
	public function getOrder($order_id) {
        ...
        'shipping_apartment'      => $order_query->row['shipping_apartment'],
        'shipping_level'          => $order_query->row['shipping_level'],
        'shipping_city_id'        => $order_query->row['shipping_city_id'],
        'shipping_post_id'        => $order_query->row['shipping_post_id'],
        'shipping_address_id'     => $order_query->row['shipping_address_id'],
        'shipping_post'           => $order_query->row['shipping_post'],
        'shipping_poshtomat'      => $order_query->row['shipping_poshtomat'],
		    'shipping_poshtomat_id'   => $order_query->row['shipping_poshtomat_id'],
		    'shipping_comment'        => $order_query->row['shipping_comment'],
        'address'                 => $order_query->row['address'],
        'client_address_id'       => $order_query->row['client_address_id'],
        'customer_cod_guid'       => $order_query->row['customer_cod_guid'],
				...
				'invoce_erp'              => $order_query->row['invoce_erp']
			);
		} else {
			return false;
		}
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
        ...
        //++Andrey
        //тут скоріше оновлює залишки після замовлення
//				foreach($order_products as $order_product) {
//					$this->db->query("UPDATE `" . DB_PREFIX . "product` SET quantity = (quantity + " . (int)$order_product['quantity'] . ") WHERE product_id = '" . (int)$order_product['product_id'] . "' AND subtract = '1'");
//
//					$order_options = $this->getOrderOptions($order_id, $order_product['order_product_id']);
