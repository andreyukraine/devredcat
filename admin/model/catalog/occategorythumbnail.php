...
class ModelCatalogOccategorythumbnail extends Model
{
  public function installCategoryThumbnail()
  {
    ...
    // Home Thumbnail Image
    $check_sql4 = "SHOW COLUMNS FROM `" . DB_PREFIX . "category` LIKE 'featured_color'";
    $query4 = $this->db->query($check_sql4);
    if ($query4->rows) {
      $flag = true;
      array_push($cate_exist, 'Featured color');
    } else {
      $sql = "ALTER TABLE `" . DB_PREFIX . "category` ADD `featured_color` varchar(255) DEFAULT NULL";
      $this->db->query($sql);
    }
  ...
  public function editCategoryThumbnail($category_id, $data)
  {
    ...
    $sql4 = "UPDATE " . DB_PREFIX . "category SET featured_color = '" . $this->db->escape($data['featured_color']) . "' WHERE category_id = '" . (int)$category_id . "'";
  ...
  {
    $sql = "SELECT c1.thumbnail_image, c1.homethumb_image, c1.featured, c1.featured_color, cp.category_id AS category_id, GROUP_CONCAT(cd1.name ORDER BY cp.level SEPARATOR '&nbsp;&nbsp;&gt;&nbsp;&nbsp;') AS name, c1.parent_id, c1.sort_order FROM " . DB_PREFIX . "category_path cp LEFT JOIN " . DB_PREFIX . "category c1 ON (cp.category_id = c1.category_id) LEFT JOIN " . DB_PREFIX . "category c2 ON (cp.path_id = c2.category_id) LEFT JOIN " . DB_PREFIX . "category_description cd1 ON (cp.path_id = cd1.category_id) LEFT JOIN " . DB_PREFIX . "category_description cd2 ON (cp.category_id = cd2.category_id) WHERE cd1.language_id = '" . (int)$this->config->get('config_language_id') . "' AND cd2.language_id = '" . (int)$this->config->get('config_language_id') . "'";
