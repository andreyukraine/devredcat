...
class ModelCatalogOption extends Model {
	public function addOption($data) {
      ...
      if (empty($value['slug'])){
        ...
        $slug = $this->model_design_seo_url->translit($value['name']);
      }else{
        $slug = $value['slug'];
      }
			$this->db->query("INSERT INTO " . DB_PREFIX . "option_description SET option_id = '" . (int)$option_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', slug = '" . $this->db->escape($slug) . "'");
		}
  ...
  public function getProductOptionValueId($product_id, $option_id, $option_value_id){
	...
	public function getOptionDescriptions($option_id) {
		...
		foreach ($query->rows as $result) {
			$option_data[$result['language_id']] = array('name' => $result['name'], 'slug' =>$result['slug']);
		}
