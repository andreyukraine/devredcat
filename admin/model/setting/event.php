...
class ModelSettingEvent extends Model {
  ...
  public function editEvent($event_id, $code, $trigger, $action, $status = 1, $sort_order = 0) {
    $this->db->query("UPDATE `" . DB_PREFIX . "event` SET `code` = '". $code ."', `trigger` = '". $trigger ."', `action` = '". $action ."', `status` = '". (int)$status ."', `sort_order` = '". $sort_order ."' WHERE event_id = '" . (int)$event_id . "'");
  }
