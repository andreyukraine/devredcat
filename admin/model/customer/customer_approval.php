...
class ModelCustomerCustomerApproval extends Model {
	public function getCustomerApprovals($data = array()) {
		$sql = "SELECT *, CONCAT(c.`firstname`, ' ', c.`lastname`) AS name, cgd.`name` AS customer_group, ca.`type`, c.zone_id as zone_id, c.telephone as telephone FROM `" . DB_PREFIX . "customer_approval` ca LEFT JOIN `" . DB_PREFIX . "customer` c ON (ca.`customer_id` = c.`customer_id`) LEFT JOIN `" . DB_PREFIX . "customer_group_description` cgd ON (c.`customer_group_id` = cgd.`customer_group_id`) WHERE cgd.`language_id` = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function getTotalCustomerApprovals($data = array()) {
		...
		if (!empty($data['zone_id'])) {
			$implode[] = "c.zone_id IN (" . implode(',', array_map('intval', $data['zone_id'])) . ")";
		}

		if (!empty($data['customer_group_id'])) {
			$implode[] = "c.customer_group_id IN (" . implode(',', array_map('intval', $data['customer_group_id'])) . ")";
		}
