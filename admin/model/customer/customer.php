...
class ModelCustomerCustomer extends Model {
	public function addCustomer($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', zone_id = '" . (int)$data['zone_id'] . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : json_encode(array())) . "', newsletter = '" . (int)$data['newsletter'] . "', salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', status = '" . (int)$data['status'] . "', safe = '" . (int)$data['safe'] . "', date_added = NOW()");
	...
	public function editCustomer($customer_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', zone_id = '" . (int)$data['zone_id'] . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : json_encode(array())) . "', newsletter = '" . (int)$data['newsletter'] . "', status = '" . (int)$data['status'] . "', safe = '" . (int)$data['safe'] . "' WHERE customer_id = '" . (int)$customer_id . "'");
  ...
  public function getPrice($customer_price_id)
  {
    $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer_price WHERE customer_price_id = '" . (int)$customer_price_id . "'");
    return $query->row;
  }
  ...
  public function getPriceGroups($customer_id){
    $price_groups = array();

    $query_address = $this->db->query("SELECT address_id FROM " . DB_PREFIX . "address WHERE customer_id = '" . (int)$customer_id . "'");

    foreach ($query_address->rows as $address) {
      $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer_address_price_groups WHERE customer_id = '" . (int)$customer_id . "' AND address_id = '" . (int)$address['address_id'] . "'");
      foreach ($query->rows as $result) {
        ...
        $price_groups[$address['address_id']][] = array(
  ...
  private function getPriceGroup($price_group_id){
	...
	public function getAddresses($customer_id) {
		...
		$query = $this->db->query("SELECT address_id, customer_price_id  FROM " . DB_PREFIX . "address WHERE customer_id = '" . (int)$customer_id . "'");

		foreach ($query->rows as $result) {
			$address_info = $this->getAddress($result['address_id']);
      $price_info = $this->getPrice($result['customer_price_id']);
