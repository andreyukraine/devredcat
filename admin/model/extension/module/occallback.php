...
class ModelExtensionModuleOccallback extends Model {
    public function getCall($request_id) {
        $query = $this->db->query("SELECT DISTINCT * FROM `" . DB_PREFIX . "occallback` WHERE request_id = '" . (int)$request_id . "'");
        
        return $query->row;
    }
    
    public function deleteCall($request_id) {
        $this->db->query("DELETE FROM `" . DB_PREFIX . "occallback` WHERE request_id = '" . (int)$request_id . "'");
    }
    
    public function deleteAllCallArray() {
        $this->db->query("DELETE FROM `" . DB_PREFIX . "occallback`");
    }
    
    public function getCallArray($data = array()) {
        $sql = "SELECT * FROM `" . DB_PREFIX . "occallback` ";
    ...
    public function getTotalCallArray($data = []) {
        $sql = "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "occallback`";
    ...
    public function addProcessed($request_id, $processed_status = 0) {
        $this->db->query("UPDATE `" . DB_PREFIX . "occallback` SET `processed` = '" . (int)$processed_status . "' WHERE request_id = '" . (int)$request_id . "'");

		return $this->db->countAffected() ? true : false;
    }
    
    public function makeDB() {
        $sql = "CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "occallback` ( ";
    ...
    public function deleteDB() {
        $this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "occallback`;");
    }
}
