...
class ModelExtensionModuleDiscount extends Model {
			...
			foreach ($data as $key) {
				...
				switch ($table) {
					...
					case 'manufacturer':
						...
						break;
					case 'product':
						$sql = "INSERT INTO " . DB_PREFIX . $table . "_discount SET " . $table  . "_id = '" . (int)$key[$id] . "', customer_group_id = '" . (int)$key['customer_group_id'] . "', quantity = '" . (int)$key['quantity'] . "', percentage = '" . (float)$key['percentage'] . "', priority = '" . (int)$key['priority'] . "', price = '" . (float)$key['price'] . "',   options = '" . $key['options']. "', options_value = '" . $key['options_value']. "', date_start = '" . $this->db->escape($key['date_start']) . "', date_end = '" . $this->db->escape($key['date_end']) . "'";
						break;
					case 'volume':
  ...
  public function getAccumulativeDisconts($table) {
    $query = $this->db->query("SELECT * FROM " . DB_PREFIX . $table . "_discount ORDER BY amount_from");
    return $query->rows;
  }

  public function setAccumulativeDisconts($data, $table) {
    $this->db->query("TRUNCATE TABLE " . DB_PREFIX . $table . "_discount;");
    foreach ($data as $item){
      $sql = "INSERT INTO " . DB_PREFIX . $table . "_discount SET `amount_from` = '" . (int)$item['amount_from'] . "', `amount_to` = '" . (int)$item['amount_to'] . "', `percent` = '" . (int)$item['percent'] . "'";
      $this->db->query($sql);
    }
    return $data;
  }
