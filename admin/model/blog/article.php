...
class ModelBlogArticle extends Model {
	public function addArticle($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "article SET status = '" . (int)$data['status'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', noindex = '" . (int)$data['noindex'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_added = NOW()");
		...
		if (isset($data['article_seo_url'])) {
          ...
          if (empty($keyword)) {
            ...
            $translit_name = $this->model_design_seo_url->translit($data['article_description'][$language_id]['name']);
	...
	public function editArticle($article_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "article SET status = '" . (int)$data['status'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', noindex = '" . (int)$data['noindex'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_modified = NOW() WHERE article_id = '" . (int)$article_id . "'");
