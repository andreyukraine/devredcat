...
class ModelSearchSearch extends Model {
  ...
  public function getProducts($data = array()) {
    $search_query = '%' . $this->db->escape(preg_replace('/["\'%_]/', '', $data['query'])) . '%';

    // Экранируем запрос, добавляем %
    //$search_query = '%' . $this->db->escape($data['query']) . '%';

    $sql = "SELECT p.product_id, pd.name, pq.ean as model, p.image
            FROM " . DB_PREFIX . "product p
            LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)
            LEFT JOIN " . DB_PREFIX . "product_quantity pq ON (pq.product_id = pd.product_id)
            WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "'
                AND ( REPLACE(pd.name, '\"', '') LIKE '" . $search_query . "'
                      OR REPLACE(pq.ean, '\"', '') LIKE '" . $search_query . "'
                      OR REPLACE(pq.sku, '\"', '') LIKE '" . $search_query . "'
                    )
