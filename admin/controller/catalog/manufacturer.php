...
class ControllerCatalogManufacturer extends Controller {
    ...
    if (isset($this->request->post['in_slide'])) {
      $data['in_slide'] = $this->request->post['in_slide'];
    } elseif (!empty($manufacturer_info)) {
      $data['in_slide'] = $manufacturer_info['in_slide'];
    } else {
      $data['in_slide'] = 1;
    }

    if (isset($this->request->post['in_footer'])) {
      $data['in_footer'] = $this->request->post['in_footer'];
    } elseif (!empty($information_info)) {
      $data['in_footer'] = $information_info['in_footer'];
    } else {
      $data['in_footer'] = 0;
    }
    ...
    foreach ($data['manufacturer_store'] as $store_id => $store) {
      foreach ($data['languages'] as $lang) {
        ...
        $temlate_brand_title = $this->model_catalog_manufacturer->getTemplateSeoManufacturer($lang['language_id'], $store_id, "config_gen_seo_brand_gpt_h1");
        if (!empty($temlate_brand_title['value'])) {
          $data['gpt_tmpl'][$lang['language_id']]["input-meta-h1"] = str_replace(array_keys($oct_replace), array_values($oct_replace), addslashes(str_replace("'", "", $temlate_brand_title['value'])));
        }
        //meta title
        $data['gpt_tmpl'][$lang['language_id']]["input-meta-title"] = "";
        $temlate_brand_title = $this->model_catalog_manufacturer->getTemplateSeoManufacturer($lang['language_id'], $store_id, "config_gen_seo_brand_gpt_title");
        if (!empty($temlate_brand_title['value'])) {
          $data['gpt_tmpl'][$lang['language_id']]["input-meta-title"] = str_replace(array_keys($oct_replace), array_values($oct_replace), addslashes(str_replace("'", "", $temlate_brand_title['value'])));
        }
        //meta meta_description
        $data['gpt_tmpl'][$lang['language_id']]["input-meta-description"] = "";
        $temlate_brand_meta_description = $this->model_catalog_manufacturer->getTemplateSeoManufacturer($lang['language_id'], $store_id, "config_gen_seo_brand_gpt_meta_description");
        if (!empty($temlate_brand_meta_description['value'])) {
          $data['gpt_tmpl'][$lang['language_id']]["input-meta-description"] = str_replace(array_keys($oct_replace), array_values($oct_replace), addslashes(str_replace("'", "", $temlate_brand_meta_description['value'])));
        }
        //meta description
        $data['gpt_tmpl'][$lang['language_id']]["input-description"] = "";
        $temlate_brand_description = $this->model_catalog_manufacturer->getTemplateSeoManufacturer($lang['language_id'], $store_id, "config_gen_seo_brand_gpt_description");
        if (!empty($temlate_brand_description['value'])) {
          $data['gpt_tmpl'][$lang['language_id']]["input-description"] = str_replace(array_keys($oct_replace), array_values($oct_replace), addslashes(str_replace("'", "", $temlate_brand_description['value'])));
        }
      }
    }
    //--Andrey
