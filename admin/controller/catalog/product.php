...
class ControllerCatalogProduct extends Controller
{
  ...
  protected function getList()
  {
    ...
    foreach ($results as $result) {
      ...
      if (isset($result['organization_id'])){
        $this->load->model('extension/module/ocorganization');
        $organization_info = $this->model_extension_module_ocorganization->getOrganizationId($result['organization_id']);
  ...
  protected function getForm()
  {
    ...
    if (!empty($data['warehouses'])) {
      foreach ($data['warehouses'] as &$warehouse) {
        ...
        if (isset($this->request->get['product_id'])) {
          $product_warehouse = $this->model_catalog_product->getProductWarehouse($this->request->get['product_id'], $warehouse['warehouse_id']);
          if (!empty($product_warehouse)) {
            foreach ($product_warehouse as $warehous) {
              $d = array();

              foreach ($product_warehouse as $o) {

                $opts = array();
                $qty_opts = json_decode($o['options'], true);
                if (!empty($qty_opts)) {
                  $product_options = $this->model_catalog_product->getProductOptions($this->request->get['product_id']);
                  if (!empty($product_options)) {
                    foreach ($product_options as &$option) {
                      if (array_key_exists($option['option_id'], $qty_opts)) {
                        foreach ($option['product_option_value'] as &$v) {
                          $v["selected"] = false;
                          if ($qty_opts[$option['option_id']] == $v['option_value_id']) {
                            $v["selected"] = true;
                          }
                          unset($v);
                        }
                ...
                $d[] = array(
                  ...
                  'quantity' => $o['quantity'],
                  'price' => $o['price'],
                  'price_base' => $o['price_base']
                );

              }
            }
            $warehouse['options'] = $d;
            $warehouse['count_options'] = count($d);
          }
        }
      }
    }
    ...
    //++Andrey
    $data['gpt_key'] = $this->config->get('config_key_gpt');
    $data['gpt_model'] = $this->config->get('config_model_gpt');

    foreach ($data['product_store'] as $store_id => $store) {
      foreach ($data['languages'] as $lang) {
        ...
        //meta h1
        $data['gpt_tmpl'][$lang['language_id']]["input-meta-h1"] = "";
        $temlate_prod_title = $this->model_catalog_product->getTemplateSeoProduct($lang['language_id'], $store_id, "config_gen_seo_prod_gpt_h1");
        if (!empty($temlate_prod_title['value'])) {
          $data['gpt_tmpl'][$lang['language_id']]["input-meta-h1"] = str_replace(array_keys($oct_replace), array_values($oct_replace), addslashes(str_replace("'", "", $temlate_prod_title['value'])));
        }
        //meta title
        $data['gpt_tmpl'][$lang['language_id']]["input-meta-title"] = "";
        $temlate_prod_title = $this->model_catalog_product->getTemplateSeoProduct($lang['language_id'], $store_id, "config_gen_seo_prod_gpt_title");
        if (!empty($temlate_prod_title['value'])) {
          $data['gpt_tmpl'][$lang['language_id']]["input-meta-title"] = str_replace(array_keys($oct_replace), array_values($oct_replace), addslashes(str_replace("'", "", $temlate_prod_title['value'])));
        }
        //meta meta_description
        $data['gpt_tmpl'][$lang['language_id']]["input-meta-description"] = "";
        $temlate_prod_meta_description = $this->model_catalog_product->getTemplateSeoProduct($lang['language_id'], $store_id, "config_gen_seo_prod_gpt_meta_description");
        if (!empty($temlate_prod_meta_description['value'])) {
          $data['gpt_tmpl'][$lang['language_id']]["input-meta-description"] = str_replace(array_keys($oct_replace), array_values($oct_replace), addslashes(str_replace("'", "", $temlate_prod_meta_description['value'])));
        }
        //meta description
        $data['gpt_tmpl'][$lang['language_id']]["input-description"] = "";
        $temlate_prod_description = $this->model_catalog_product->getTemplateSeoProduct($lang['language_id'], $store_id, "config_gen_seo_prod_gpt_description");
        if (!empty($temlate_prod_description['value'])) {
          $data['gpt_tmpl'][$lang['language_id']]["input-description"] = str_replace(array_keys($oct_replace), array_values($oct_replace), addslashes(str_replace("'", "", $temlate_prod_description['value'])));
        }
      }
    }

    $this->load->model('catalog/product_status');
    $data['product_statuses'] = $this->model_catalog_product_status->getProductStatuses();

    if (isset($this->request->post['product_status_id'])) {
      $data['product_status_id'] = $this->request->post['product_status_id'];
    } elseif (!empty($product_info)) {
      $data['product_status_id'] = $product_info['product_status_id'];
    } else {
      $data['product_status_id'] = 0;
    }
    ...
    $groups = $this->model_catalog_product->getPriceGroups();
    if ($groups) {
    ...
    if (isset($this->request->post['price_group_id'])) {
      ...
      $data['price_group_id'] = $product_info['price_group_id'];
    } else {
    ...
    foreach ($product_options as $product_option) {
      ...
      if (isset($product_option['product_option_value'])) {
        ...
        foreach ($product_option['product_option_value'] as $product_option_value) {
          ...
          //++Andrey
          $images_opt = $this->model_catalog_product->getProductOptImages($this->request->get['product_id'], $product_option_value['product_option_value_id']);
          ...
          $product_option_value_data[] = array(
            ...
            'composition' => $product_option_value['composition']
          );
        }
      }
    ...
    //++Andrey
    if (isset($this->request->get['product_id'])) {
      $dd = $this->model_catalog_product->getProductOptions($this->request->get['product_id']);
      $opts = array();
      if (!empty($dd)) {
        foreach ($dd as $option) {
          $v = array();
  ...
  public function autocomplete()
  {
    ...
    if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model'])) {
      ...
      if (!empty($this->request->get['filter_name']) || !empty($this->request->get['filter_model'])) {
        ...
        if (strlen($filter_name) > 3 || strlen($filter_model) >= 2) {
          $filter_data = array(
            'filter_name' => $filter_name,
            'filter_model' => $filter_model,
            'start' => 0,
