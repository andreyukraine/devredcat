...
class ControllerDesignBanner extends Controller {
    ...
    $categories = array();
		...
		if (isset($this->request->post['category'])) {
			...
			$categories = $this->model_design_banner->getCategories($this->request->get['banner_id']);
		} else {
			$categories = array();
		}

		if (isset($this->request->post['manufactures'])) {
			$manufactures = $this->request->post['manufactures'];
		} elseif (isset($this->request->get['banner_id'])) {
			$manufactures = $this->model_design_banner->getManufactures($this->request->get['banner_id']);
		} else {
			$manufactures = array();
		}
    ...
    if ($categories) {
      ...
      foreach ($categories as $category_id) {
        ...
        if (is_file(DIR_IMAGE . $banner_image['image_mob'])) {
          $image_mob = $banner_image['image_mob'];
          $thumb_mob = $banner_image['image_mob'];
        } else {
          $image = '';
          $thumb_mob = 'no_image.png';
        }
          ...
          'thumb_mob'  => $this->model_tool_image->resize($thumb_mob, 100, 100),
