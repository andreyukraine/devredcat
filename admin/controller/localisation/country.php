...
class ControllerLocalisationCountry extends Controller {
	...
	protected function getList() {
		...
		$data['import'] = $this->url->link('localisation/country/import', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['import_regions'] = $this->url->link('localisation/zone/import', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['delete'] = $this->url->link('localisation/country/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);

		$data['button_import_regions'] = $this->language->get('button_import_regions');
		...
		foreach ($results as $result) {
			$data['countries'][] = array(
				...
				'import_regions' => $this->url->link('localisation/zone/import', 'user_token=' . $this->session->data['user_token'] . '&country_id=' . $result['country_id'] . $url, true)
			);
		}
	...
	public function import() {
		...
		if (!$api_key) {
			...
			$this->response->redirect($this->url->link('localisation/country', 'user_token=' . $this->session->data['user_token'], true));
		}
		...
		if (!empty($countries_data['success']) && !empty($countries_data['data'])) {
			$total_updated = 0;
			$total_added = 0;

			foreach ($countries_data['data'] as $country) {
				$name = $country['Description'];
				$iso_code_2 = $country['Code'];
				$ref = $country['Ref'];

				// 1. Спробувати знайти за Ref
				$check_query = $this->db->query("SELECT country_id FROM " . DB_PREFIX . "country WHERE np_country_ref = '" . $this->db->escape($ref) . "' LIMIT 1");

				if ($check_query->num_rows) {
					$this->db->query("UPDATE " . DB_PREFIX . "country SET name = '" . $this->db->escape($name) . "', iso_code_2 = '" . $this->db->escape($iso_code_2) . "' WHERE country_id = '" . (int)$check_query->row['country_id'] . "'");
					$total_updated++;
				} else {
					// 2. Спробувати знайти за ISO кодом
					$iso_query = $this->db->query("SELECT country_id FROM " . DB_PREFIX . "country WHERE iso_code_2 = '" . $this->db->escape($iso_code_2) . "' LIMIT 1");

					if ($iso_query->num_rows) {
						$this->db->query("UPDATE " . DB_PREFIX . "country SET np_country_ref = '" . $this->db->escape($ref) . "', name = '" . $this->db->escape($name) . "' WHERE country_id = '" . (int)$iso_query->row['country_id'] . "'");
						$total_updated++;
					} else {
						// 3. Додати нову країну
						$this->db->query("INSERT INTO " . DB_PREFIX . "country SET name = '" . $this->db->escape($name) . "', iso_code_2 = '" . $this->db->escape($iso_code_2) . "', iso_code_3 = '', address_format = '', postcode_required = 0, status = 1, np_country_ref = '" . $this->db->escape($ref) . "'");
						$total_added++;
					}
				}
			}

			$this->session->data['success'] = sprintf('Success: Updated %d countries and added %d new countries!', $total_updated, $total_added);
		} else {
			$this->session->data['error_warning'] = 'Failed to fetch countries from Nova Poshta!';
		}
