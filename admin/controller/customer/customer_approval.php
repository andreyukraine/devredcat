...
class ControllerCustomerCustomerApproval extends Controller {
	...
	public function customer_approval() {
		...
		if ($this->user->getGroupId() != 1) {
			if (!empty($user_group_info['permission']['zone'])) {
				$zone_id = $user_group_info['permission']['zone'];
			} else {
				$zone_id = array(-1);
			}

			if (!empty($user_group_info['permission']['customer_group'])) {
				$customer_group_id = $user_group_info['permission']['customer_group'];
			} else {
    ...
    $this->load->model('localisation/zone');
		...
		$filter_data_custom = array(
			'sort'  => 'cf.sort_order',
			'order' => 'ASC'
		);

		$custom_fields = $this->model_customer_custom_field->getCustomFields($filter_data_custom);
      ...
      foreach ($custom_fields as $custom_field) {
        if ($custom_field['type'] == 'file') {
          ...
          if (isset($account_custom_field[$custom_field['custom_field_id']])) {
            $code = $account_custom_field[$custom_field['custom_field_id']];
          }
