...
class ControllerCustomerCustomer extends Controller {
	...
	protected function getList() {
      ...
      if (!empty($zone)){
		...
		$data['sort_customer_id'] = $this->url->link('customer/customer', 'user_token=' . $this->session->data['user_token'] . '&sort=customer_id' . $url, true);
		...
		foreach ($custom_fields as $custom_field) {
			if ($custom_field['type'] == 'file') {
				...
				if ($code) {
					...
					if ($upload_info) {
						$data['download'][$custom_field['custom_field_id']] = $this->url->link('tool/upload/download', 'user_token=' . $this->session->data['user_token'] . '&code=' . $upload_info['code'], true);
					}
				}
			}
		}
    ...
    $data['address_price_groups'] = $this->model_customer_customer->getPriceGroups($this->request->get['customer_id']);
