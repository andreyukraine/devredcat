...
class ControllerSaleOrder extends Controller {
    ...
    $this->load->model('customer/customer');
		...
		$user_query = $this->db->query("SELECT u.firstname, u.lastname, u.code_erp, ug.permission FROM `" . DB_PREFIX . "user` u LEFT JOIN `" . DB_PREFIX . "user_group` ug ON (u.user_group_id = ug.user_group_id) WHERE u.status = '1'");
		
		$all_users = array();
		foreach ($user_query->rows as $user_row) {
			$permission = json_decode($user_row['permission'], true);
			$zones = isset($permission['zone']) ? $permission['zone'] : array();
			
			$all_users[] = array(
				'name'     => $user_row['firstname'] . ' ' . $user_row['lastname'],
				'code_erp' => $user_row['code_erp'],
				'zones'    => $zones
			);
		}
      ...
      $address = $this->model_customer_customer->getAddress($result['client_address_id']);
      ...
      if ($customer) {
        $zone_info = $this->model_localisation_zone->getZone($customer['zone_id']);
        if ($zone_info != null){
          $zone_name = $zone_info['name'];
        }

        foreach ($all_users as $user) {
          if (in_array($customer['zone_id'], $user['zones'])) {
            $managers[] = array(
              'name'     => $user['name'],
              'code_erp' => $user['code_erp']
            );
          }
        }
      }
	...
	public function getForm() {
		...
		if (!empty($order_info)) {
			...
			$data['shipping_city'] = $order_info['shipping_city'];
			$data['shipping_country_id'] = $order_info['shipping_country_id'] ? $order_info['shipping_country_id'] : '222';
			$data['shipping_zone_id'] = $order_info['shipping_zone_id'];
			$data['payment_country_id'] = $order_info['payment_country_id'] ? $order_info['payment_country_id'] : '222';
			$data['payment_zone_id'] = $order_info['payment_zone_id'];
			$data['shipping_house'] = $order_info['shipping_house'];
			$data['shipping_apartment'] = $order_info['shipping_apartment'];
			$data['shipping_method'] = $order_info['shipping_method'];
			$data['shipping_code'] = $order_info['shipping_code'];
			$data['client_address_id'] = $order_info['client_address_id'];
			$data['customer_cod_guid'] = $order_info['customer_cod_guid'];
			$data['address'] = $order_info['address'];
			$data['shipping_level'] = $order_info['shipping_level'];
			$data['shipping_city_id'] = $order_info['shipping_city_id'];
			$data['shipping_city_delivery_id'] = $order_info['shipping_city_delivery_id'];
			$data['shipping_post_id'] = $order_info['shipping_post_id'];
			$data['shipping_address_id'] = $order_info['shipping_address_id'];
			$data['shipping_post'] = $order_info['shipping_post'];
			$data['shipping_poshtomat'] = $order_info['shipping_poshtomat'];
			$data['shipping_poshtomat_id'] = $order_info['shipping_poshtomat_id'];
			$data['shipping_comment'] = $order_info['shipping_comment'];
			$data['payment_city_id'] = ''; // Not in DB

			$this->load->model('customer/customer');
			$address_info = $this->model_customer_customer->getAddress($order_info['client_address_id']);
			if ($address_info) {
			...
			$data['shipping_country_id'] = '222';
			$data['shipping_zone_id'] = '';
			$data['payment_country_id'] = '222';
			$data['payment_zone_id'] = '';
			$data['shipping_house'] = '';
			$data['shipping_apartment'] = '';
			$data['client_address_id'] = '';
			$data['customer_cod_guid'] = '';
			$data['address'] = '';
			$data['shipping_level'] = '';
			$data['shipping_city_id'] = '';
			$data['shipping_post_id'] = '';
			$data['shipping_address_id'] = '';
			$data['shipping_post'] = '';
			$data['shipping_poshtomat'] = '';
			$data['shipping_poshtomat_id'] = '';
			$data['shipping_comment'] = '';

			$data['shipping_city_delivery_id'] = '';
	...
	public function info() {
		...
		if ($order_info) {
			...
			// Shipping Address
      $this->load->model('customer/customer');
      $address = $this->model_customer_customer->getAddress($order_info['client_address_id']);

      $data['point'] = $address['firstname'];
      $data['shipping_comment'] = $address['comment'];

      $data['show_np_customer'] = false;
      $data['np_customer_name'] = $address['np_customer_name'];
      $data['np_customer_lastname'] = $address['np_customer_lastname'];
      $data['np_customer_phone'] = $address['np_customer_phone'];
      if (!empty($data['np_customer_name']) || !empty($data['np_customer_lastname']) || !empty($data['np_customer_phone'])){
        $data['show_np_customer'] = true;
      }

      $data['shipping_address'] = $order_info['address'];
	...
	public function sendManualPush() {
		$json = array();
					...
					if ($target_group_ids) {
						...
						if ($user_query->num_rows) {
							$push_data = [
								...
								'message'     => 'Замовлення №' . $order_info['order_id'] . ' від ' . $order_info['firstname'] . ' ' . $order_info['lastname'],
							...
							foreach ($user_query->rows as $user) {
								if (!empty($user['fcm_token'])) {
									...
									if (isset($result['error'])) {
										$this->log->write("FCM MANUAL ERROR: Failed to send to user " . $user['username'] . " (ID: " . $user['user_id'] . "). Error: " . $result['error'] . " Response: " . $result['response']);
									} else {
										$this->log->write("FCM MANUAL SUCCESS: Sent to user " . $user['username'] . " (ID: " . $user['user_id'] . "). Response: " . $result['response']);
										$sent_count++;
									}
								}
							}
							...
							if ($sent_count > 0) {
								$json['success'] = 'Push notifications sent to ' . $sent_count . ' users';
							} else {
								$json['error'] = 'Could not send push notifications to any users';
							}
						} else {
							$json['error'] = 'No users found with fcm_token in this zone';
						}
					} else {
						$json['error'] = 'No user groups found with permission for this zone';
					}
				} else {
					$json['error'] = 'Customer zone not found';
				}
			} else {
