...
class ControllerExtensionModuleCustom extends Controller {
    ...
    if (isset($this->request->post['module_custom_fastorder'])) {
      $data['module_custom_fastorder'] = $this->request->post['module_custom_fastorder'];
    } else {
      $data['module_custom_fastorder'] = $this->config->get('module_custom_fastorder');
    }

    if (isset($this->request->post['module_custom_split_order_store'])) {
			$data['module_custom_split_order_store'] = $this->request->post['module_custom_split_order_store'];
		} else {
			$data['module_custom_split_order_store'] = $this->config->get('module_custom_split_order_store');
		}

    if (isset($this->request->post['module_custom_check_qty_cart_by_store'])) {
      $data['module_custom_check_qty_cart_by_store'] = $this->request->post['module_custom_check_qty_cart_by_store'];
    } else {
      $data['module_custom_check_qty_cart_by_store'] = $this->config->get('module_custom_check_qty_cart_by_store');
    }
		...
		$data['module_custom_shipping']['list'] = $this->getShippingMethodList();
	...
	protected function validate() {
		...
		if (!isset($this->request->post['module_custom_payment']['methods']) || count($this->request->post['module_custom_payment']['methods']) == 0) {
			...
			foreach($this->request->post['module_custom_payment']['methods'] as $method){
				if (!in_array($method['code'], $method_temp)) {
					$method_temp[] = $method['code'];
				} else {
    ...
    $result = $this->model_setting_setting->getSetting('module_custom');
      ...
      foreach($result['module_custom_shipping']['methods'] as $delivery) {
        if ($delivery['code'] == $method) {
          $methods[$key]['sort'] = $delivery['sort'] ?? 0;
          $methods[$key]['minimum'] = $delivery['minimum'] ?? 0;
          $methods[$key]['minimum_postomat'] = $delivery['minimum_postomat'] ?? array();
          $methods[$key]['minimum_address'] = $delivery['minimum_address'] ?? array();
          $methods[$key]['free'] = $delivery['free'] ?? 0;
          $methods[$key]['cost'] = $delivery['cost'] ?? 0;
          $methods[$key]['free_postomat'] = $delivery['free_postomat'] ?? array();
          $methods[$key]['cost_postomat'] = $delivery['cost_postomat'] ?? array();
          $methods[$key]['free_address'] = $delivery['free_address'] ?? array();
          $methods[$key]['cost_address'] = $delivery['cost_address'] ?? array();
          $methods[$key]['descs'] = $delivery['descs'] ?? array();
          $methods[$key]['title'] = $delivery['title'] ?? array();
          $methods[$key]['customer_group'] = $delivery['customer_group'] ?? array();
        }
      }

			$methods[$key]['code'] = $method;
			$methods[$key]['label'] = $this->language->get('heading_title');
      $methods[$key]['payments'] = $this->getShippingPaymentList($method, $result);
		}
		return $methods;
	}

  private function getShippingPaymentList($code, $result)
  {
    $res = array();
    foreach($result['module_custom_shipping']['methods'] as $delivery){
      if ($delivery['code'] == $code){
        foreach($delivery['payments'] as $payment_code) {
