...
class ControllerExtensionTotalSet extends Controller {
    ...
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            $this->model_setting_setting->editSetting('total_set', $this->request->post);
        ...
        if (isset($this->request->post['total_set_status'])) {
            $data['total_set_status'] = $this->request->post['total_set_status'];
        } else {
            $data['total_set_status'] = $this->config->get('total_set_status');
        }

        if (isset($this->request->post['set_sort_order'])) {
            $data['set_sort_order'] = $this->request->post['set_sort_order'];
        } else {
            $data['set_sort_order'] = $this->config->get('set_sort_order');
        }
