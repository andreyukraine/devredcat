...
class SeoPro {
    ...
    public function prepareRoute($parts) {
        ...
        if (!empty($parts) && is_array($parts)) {
            ...
            $parts_filtered = array_values(array_filter($parts));
            if (count($parts_filtered) === 1) {
                $first = $parts_filtered[0];
                if ($first === 'blog' || $first === 'блог' || utf8_strtolower($first) === 'blog') {
            ...
            foreach($parts as $id => $part) {
                ...
                if($parts[$id]) {
                    ...
                    $query = $this->getQueryByKeyword($parts[$id], null, $is_brand);
                    $is_brand = false;

                    if (($parts[$id] === 'blog' || $parts[$id] === 'блог') && !$query) {
                        $this->request->get['route'] = 'blog/blog';
                        unset($parts[$id]);
                        continue;
                    }
                    ...
                    if(!empty($url[0])) {
                        ...
                        if(!in_array($url[0], ['category_id', 'product_id', 'manufacturer_id', 'information_id', 'article_id', 'blog_category_id', 'akcia_id'])) {
                            if (count($url) == 1) {
                                $this->request->get['route'] = $query;
                                unset($parts[$id]);
                                continue;
                            }
                            if (count($url) == 2 && $url[0] == 'route') {
                                unset($parts[$id]);
                                continue;
                            }
                            return $parts;
            ...
            if(!$query && empty($this->request->get['route'])) {
        ...
        }elseif (isset($this->request->get['akcia_id'])) {
            //akcii
            $this->request->get['route'] = 'product/akcii/info';
    ...
    public function baseRewrite($data, $language_id) {
        ...
        switch ($data['route']) {
            ...
            case 'products/akcii':
              if (isset($data['article_id'])) {
                $route = 'products/akcii';

                unset($data);
                $data['route'] = $route;

                $akcia_id = $data['akcia_id'];
            ...
            if (!empty($rows) && (count($rows) == count($queries))) {
                foreach($queries as $i => $query) {
                    ...
                    if (strpos($query, 'manufacturer_id=') === 0) {
                        $url .= '/brand/' . rawurlencode($keyword);
                    } else {
                        $url .= '/' . rawurlencode($keyword);
                    }
                }
        ...
        if (($route == 'blog/category' || $route == 'blog/article') && $url !== null && $url !== '') {
            $blog_prefix = $this->getKeywordByQuery('blog/blog');
            if ($blog_prefix === null || $blog_prefix === '') {
                $blog_prefix = 'blog';
            }
            $url = '/' . rawurlencode($blog_prefix) . $url;
        }
            ...
            if ((!$this->keywords || empty($this->keywords) || !$this->queries || empty($this->queries))) {
                ...
                if($query->num_rows) {
                    foreach($query->rows as $row) {
                        ...
                        if (strpos($row['query'], 'manufacturer_id=') === 0) {
                            $this->queries['brand:' . $row['keyword']][$row['store_id']][$row['language_id']] = $row['query'];
                        } else {
    ...
    private function getQueryByKeyword($keyword, $language_id = null, $is_brand = false) {
        $query = null;
        ...
        $cache_keyword = ($is_brand ? 'brand:' : '') . $keyword;

        if ($this->config->get('config_seo_url_cache')){
            if (isset($this->queries[$cache_keyword][$store_id][$language_id]))
                $query = $this->queries[$cache_keyword][$store_id][$language_id];
        } else {
            $sql = "SELECT query FROM " . DB_PREFIX . "seo_url WHERE keyword = '" . $this->db->escape(trim($keyword)) . "' AND store_id = '" . $store_id . "' AND language_id = '" . $language_id . "'";
            if ($is_brand) {
